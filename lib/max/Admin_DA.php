...
class Admin_DA
{
    ...
    function addAdZone($aVariables)
    {
        ...
        if (PEAR::isError($canAddAdZone)) {
            ...
            // Add the invocation code of banner -- dwt
            $zoneid = $aVariables['zone_id'];
            if ($zoneid != 0) {
                ...
                if ($zone = $dalZones->getZoneForInvocationForm($zoneid)) {
                    ...
                    $invocationCode = $maxInvocation->buildInvocationCode($extra, true);
                    $trimCode = trim(substr($invocationCode, stripos($invocationCode, '-->')+3));
                    Admin_DA::_addEntity('invocation_code', array('bannerid' => $aVariables['ad_id'], 'invocation_code' => $trimCode));
                }
            }
        }
    ...
    function deleteAdZones($aParams, $allFields = false)
    {
        ...
        $result = SqlBuilder::_doDelete('ad_zone_assoc', $aParams);
        // author: dwt
        $codeResult = SqlBuilder::_doDelete('invocation_code', array('bannerid' => $aParams['ad_id']));
        return $result;
    }
